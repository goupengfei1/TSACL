...
class TimeFeatureEmbedding(nn.Module):
    def __init__(self, d_model, embed_type='timeF', freq='h', wo_freq=' ', dropout=0.0):
        super(TimeFeatureEmbedding, self).__init__()

        # t > h > w > d > m
        # hour dim=0, week_dim=1, day_dim=2, year_dim=3
        freq_dim_map = {'h':0, 'w':1, 'd':2, 'm':6}
        self.rm_idx = []
        ...
        freq_map = {'h': 5, 't': 6, 's': 6, 'm': 1, 'a': 1, 'w': 2, 'd': 4, 'b': 3}
        self.d_inp = freq_map[freq]

        self.tf_embed = nn.Sequential(nn.Linear(self.d_inp, d_model, bias=False))
    def forward(self, x):
        x = self.tf_embed(x)
        return x
