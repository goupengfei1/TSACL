...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.enc_embedding = DataEmbedding(self.channels, self.hidden_dims, configs.embed, configs.freq, dropout=0)
        ...
        self.res_linear = nn.Sequential(nn.Linear(configs.seq_len, configs.pred_len),
                                         nn.Dropout(0.1),)
        self.decoder = Decoder(kernel_size=configs.decoder_desize, seq_len=self.seq_len,
                               pred_len=self.pred_len,repr_dims=self.repr_dims,d_layers=1)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        repr = self.feature_extractor(enc_out).transpose(1, 2)
        ori_repr = self.repr_dropout(repr)
        repr = self.norm1(ori_repr)  # (B, T, C)
        len_out = F.gelu(repr)
        trend_outs = self.decoder(len_out)


        y_temporal = self.res_linear(short_x.permute(0,2,1)).permute(0,2,1)
